...
precmd() {
    ...
    if [ "" != "$tool_states" ]; then
        topline="┌ %{${COLOR_YELLOW_FADED}%}20%D%{${COLOR_NORM}%} %d ${tool_states}"
        #statusline=$(printf "\n│ ${tool_states}")
    ...
    if [ "0" = "$last_exit_code" ] || [ "" = "$last_exit_code" ]; then
        promptline="└ %{${COLOR_GREEN_FADED}%}%D{%H:%M:%S}%{${COLOR_NORM}%} → %{$(echo -e -n "\x1b[\x35 q")%}"
    else
        promptline="└ %{${COLOR_GREEN_FADED}%}%D{%H:%M:%S}%{${COLOR_NORM}%} (%{${COLOR_RED_FADED}%}${last_exit_code}%{${COLOR_NORM}%}) ⤳ %{$(echo -e -n "\x1b[\x35 q")%}"
    fi
}
